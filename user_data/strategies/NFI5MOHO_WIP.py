...
class NFI5MOHO_WIP(IStrategy):
    ...
    def initialize_live_tracking(self):
        """Initialize live profit/loss tracking system"""
        if self.live_tracking['start_time'] is None:
            current_time = datetime.now()
            self.live_tracking['start_time'] = current_time
            logging.info(f"🚀 Live Profit/Loss Tracking Initialized - Epochs: {self.epochs}")

    def update_live_tracking(self, current_profit: float = 0.0, trade_closed: bool = False, trade_profit: float = 0.0):
        """Update live profit/loss tracking"""
        try:
            current_time = datetime.now()
            self.initialize_live_tracking()

            # Calculate runtime
            runtime_delta = current_time - self.live_tracking['start_time']
            self.live_tracking['runtime_hours'] = runtime_delta.total_seconds() / 3600

            # Update current profit
            self.preview_data['current_profit'] = current_profit

            # Track completed trades
            if trade_closed:
                self.live_tracking['completed_trades'] += 1
                if trade_profit > 0:
                    self.live_tracking['win_trades'] += 1
                    self.live_tracking['total_profit'] += trade_profit
                    if trade_profit > self.live_tracking['best_profit']:
                        self.live_tracking['best_profit'] = trade_profit
                        logging.info(f"💰 NEW BEST TRADE PROFIT: {trade_profit:.4f} USDC!")
                else:
                    self.live_tracking['loss_trades'] += 1
                    self.live_tracking['total_loss'] += abs(trade_profit)
                    if trade_profit < self.live_tracking['worst_loss']:
                        self.live_tracking['worst_loss'] = trade_profit
                        logging.info(f"📉 NEW WORST LOSS: {trade_profit:.4f} USDC")

                # Log trade completion
                logging.info(f"🔄 TRADE COMPLETED: {trade_profit:.4f} USDC | Total Trades: {self.live_tracking['completed_trades']}")

        except Exception as e:
            logging.error(f"Error updating live tracking: {e}")

    def get_win_rate(self) -> float:
        """Calculate win rate percentage"""
        if self.live_tracking['completed_trades'] == 0:
            return 0.0
    ...
    def update_preview_data(self, dataframe: DataFrame, metadata: dict, signal_type: str = None):
        ...
        try:
            current_time = datetime.now()
            self.preview_data['total_checks'] += 1
            self.preview_data['last_update'] = current_time.strftime('%H:%M:%S')

            if signal_type == 'buy':
                self.preview_data['buy_signals'] += 1
            elif signal_type == 'sell':
                self.preview_data['sell_signals'] += 1

            # Update live tracking
            self.update_live_tracking(self.preview_data['current_profit'])

            # Log preview every 10 checks
            if self.preview_data['total_checks'] % 10 == 0:
                self.log_preview_status(metadata.get('pair', 'Unknown'))

        except Exception as e:
            logging.error(f"Error updating preview data: {e}")

    def log_preview_status(self, pair: str):
        """Log current preview status with live profit/loss info"""
        try:
            runtime_hours = self.live_tracking.get('runtime_hours', 0)
            total_profit = self.live_tracking.get('total_profit', 0)
            total_loss = self.live_tracking.get('total_loss', 0)
            net_profit = total_profit - total_loss
            completed_trades = self.live_tracking.get('completed_trades', 0)
            win_rate = self.get_win_rate()
            best_profit = self.live_tracking.get('best_profit', 0)
            worst_loss = self.live_tracking.get('worst_loss', 0)

            logging.info(
                f"📊 LIVE PREVIEW [{pair}] - "
                f"Checks: {self.preview_data['total_checks']} | "
                f"Buy: {self.preview_data['buy_signals']} | "
                f"Sell: {self.preview_data['sell_signals']} | "
                f"Runtime: {runtime_hours:.1f}h | "
                f"Epochs: {self.epochs} | "
                f"Trades: {completed_trades} | "
                f"Win Rate: {win_rate:.1f}% | "
                f"Net P/L: {net_profit:.4f} USDC | "
                f"Best: +{best_profit:.4f} | "
                f"Worst: {worst_loss:.4f}"
            )
        except Exception as e:
            logging.error(f"Error logging preview status: {e}")

    def check_conditions_and_log(self, dataframe: DataFrame, conditions: list, condition_name: str):
    ...
    def custom_sell(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # Update current profit in preview data
        self.preview_data['current_profit'] = current_profit

        # Update live tracking with current profit
        self.update_live_tracking(current_profit)
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float,
        ...
        """Confirm trade exit and track profit/loss"""
        try:
            # Calculate trade profit in USDC
            trade_profit = (rate - trade.open_rate) * amount

            # Update live tracking with completed trade
            self.update_live_tracking(trade_profit, trade_closed=True, trade_profit=trade_profit)

            # Log detailed trade info
            logging.info(
                f"🔄 TRADE EXIT CONFIRMED [{pair}] - "
                f"Profit: {trade_profit:.4f} USDC | "
                f"Rate: {rate:.6f} | "
                f"Amount: {amount:.6f} | "
            ...
            logging.error(f"Error in confirm_trade_exit: {e}")
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Update preview data and check conditions
        self.update_preview_data(dataframe, metadata)

        # Check which conditions are met and log them
        self.check_conditions_and_log(dataframe, conditions, 'BUY_CONDITION')
        ...
        if conditions:
            ...
            # Log buy signals
            if buy_signal.any():
                self.update_preview_data(dataframe, metadata, 'buy')
	...
	"""
        ...
        # Check which conditions are met and log them
        self.check_conditions_and_log(dataframe, conditions, 'SELL_CONDITION')
        ...
        if conditions:
            ...
            # Log sell signals
            if sell_signal.any():
                self.update_preview_data(dataframe, metadata, 'sell')
